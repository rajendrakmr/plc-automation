...
function validate(form: CategoryForm): FormErrors {
  ...
  if (!form.blog_cat_slug.trim()) e.blog_cat_slug = "Permalink is required.";
...
export default function EditBlogCategory() {
  ...
  useEffect(() => {
    const blog = sessionStorage.getItem("blog-category-edit");
    if (blog) {
  ...
  const handleName = (v: string) => {
    set("blog_cat_name", v);
    set("blog_cat_slug", v.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, ""));
  };

  const { loading, error, success, patch, reset ,data} = usePatch<any, { id: string }>({
    url: `/blogs/category/${form.blog_cat_id}`,
    onSuccess: () => {
  ...
  const handleSubmit = async () => {
    ...
    //     "blog_cat_name", "blog_cat_slug", "blog_cat_id",
    //   ];
    //   const first = order.find(k => errs[k]);
    //   if (first && refs[first].current) {
    //     refs[first].current.scrollIntoView({ behavior: "smooth", block: "center" });
      ...
      <div className="ap-wrap">
        <div className="ap-header">
          <div className="ap-header-left">
            <div>
              <p className="ap-title">Update Blog Category</p>
              <p className="ap-sub">Fill all mandatory fields.</p>
            </div>
          </div>
          <div className="ap-header-right">
            <Link href="/admin/blogs/category" className="ap-back">← Cancel</Link>
            <button
              className="ap-save"
