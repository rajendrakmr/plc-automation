...
function SortIcon({
  ...
  if (sortKey !== col) return <span className="af-sort-icon af-sort-idle">⇅</span>;
  return <span className="af-sort-icon af-sort-active">{sortDir === "asc" ? "↑" : "↓"}</span>;
}
...
export default function AddFeature() { 
  const [featureType, setFeatureType] = useState<FeatureTypeKey>("category");
  ...
  // ── Filters ──
  const emptyFilter: FilterState = { search: "", type: "", status: "", isSearch: false };
  ...
  const toggleAll = () => {
    if (products.length > 0 && products.every((p) => selectedIds.has(p.category_id))) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(products.map((p) => p.category_id)));
    }
  };
  ...
  const allPageChecked = products.length > 0 && products.every((p) => selectedIds.has(p.category_id));
  const someChecked = products.some((p) => selectedIds.has(p.category_id)) && !allPageChecked;
    ...
    <div className="af-page">
      ...
      <div className="af-topbar">
        ...
        <div className="af-topbar-actions">
          <Link href="/admin/products/feature" className="af-btn-cancel-top">← Cancel</Link>
                ...
                <table className="af-prod-table">
                  ...
                  <tbody>
                    {isLoading ? (
                      ...
                      products.map((p) => {
                        const checked = selectedIds.has(p.category_id);
                        return (
                          <tr
                            key={p.category_id}
                            className={`af-prod-row${checked ? " selected" : ""}`}
                            onClick={() => toggleProduct(p.category_id)}
                          >
                            <td>
